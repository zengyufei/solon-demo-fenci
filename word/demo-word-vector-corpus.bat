...
rem 1、从语料库中提取词，词之间以空格分隔，内容保存到文件 data/word.txt
if not exist data/word.txt call %EXECUTOR% org.apdplat.word.corpus.ExtractText  data/word.txt
...
if not exist data/vector.txt call %EXECUTOR% org.apdplat.word.vector.Word2Vector  data/word.txt  data/vector.txt  data/vocabulary.txt %CONTEXT_WINDOW_LENGTH% %VECTOR_LENGTH%
